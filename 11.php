...
function getMaximumSumOfAdjecentElements($bigArr, $length){
    $arrayDims = [sizeof($bigArr[0]), sizeof($bigArr)]; //horizontal size then vert size
    $maxValue = 0;
    $maxIndex = 0;
    $maxDirection = 0;

    //first, iterate over horizontals
    for($y = 0; $y < $arrayDims[1]; $y++){
        for($x = 0; $x <= ($arrayDims[0]-$length); $x++){
            $subslice = array();
            for($iter = 0; $iter < $length; $iter++){
                array_push($subslice, $bigArr[$y][$x+$iter]);
            }
            if(array_product($subslice) > $maxValue){
                $maxValue = array_product($subslice);
                $maxIndex = [$x, $y];
                $maxDirection = 0;
            }
        }
    ...
    echo("max value is ".$maxValue." at location ".implode(",",$maxIndex)." direction: ".$maxDirection."\n");

    //now, verticals
    for($x = 0; $x < $arrayDims[0]; $x++){
        for($y = 0; $y <= $arrayDims[1] - $length; $y++){
            $subslice = array();
            for($iter = 0; $iter < $length; $iter++){
                array_push($subslice, $bigArr[$y+$iter][$x]);
            }
            if(array_product($subslice) > $maxValue){
                $maxValue = array_product($subslice);
                $maxIndex = [$x, $y];
    ...
    for($x = 0; $x <= $arrayDims[0] - $length; $x++){
        for($y = 0; $y <= $arrayDims[1] - $length; $y++){
            $subslice = array();
            for($iter = 0; $iter < $length; $iter++){
                array_push($subslice, $bigArr[$y+$iter][$x+$iter]);
            }
            if(array_product($subslice) > $maxValue){
                $maxValue = array_product($subslice);
                $maxIndex = [$x, $y];
    ...
    //now right diagonals
    for($x = $length-1; $x < $arrayDims[0]; $x++){
        for($y = 0; $y <= $arrayDims[1] - $length; $y++){
            $subslice = array();
            for($iter = 0; $iter < $length; $iter++){
                array_push($subslice, $bigArr[$y+$iter][$x-$iter]);
    ...
    echo("max value is ".$maxValue." at location ".implode(",",$maxIndex)." direction: ".$maxDirection."\n");
    //i guess you could slap all the elements in an associative array to return
    //i miss structs
}

getMaximumSumOfAdjecentElements($bigArr, 4);
